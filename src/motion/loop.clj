(ns motion.loop
  ...
  through motion.cosci -> motion.governor -> commit+publish. `run!` reduces
  ...
  (:require [clojure.java.io :as io]
            ...
            [motion.judge :as judge]
            [motion.cosci :as cosci]
            [motion.governor :as governor]
            [motion.datalad :as datalad]
            [motion.ledger :as ledger]
            [motion.publish :as publish]
            [motion.aozora :as aozora]))

(def lease-path ".motion/lease")
(def state-path ".motion/state.edn")
...
(defn- fetch-result
  "One pending entry -> a motion.cosci/reflect-shaped result map, or nil if
...
(defn- asset-title [gene]
  (str/join ", " (remove nil? [(:clip-type gene) (:pacing gene)])))
...
(defn settle-round!
  ...
  motion.publisher/mock-publisher in tests."
   ...
   (let [p (persona/load-persona)
     ...
     (when winner
       (let [{:keys [result score]} winner
             ...
             asset {:id (str "motion-" (str/replace job-id #"[^a-zA-Z0-9-]" "-"))
