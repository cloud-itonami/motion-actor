(ns motion.judge
  ...
  cloud_itonami.media's own docstring): this judges the CANDIDATE'S
...
(defn score!
  ...
  failure (gateway unreachable, non-200, unparseable) — motion.cosci/reflect
  ...
  ([{:keys [prompt]} {:keys [voice tags]}
   ...
   (try
     (let [url (or url (gateway-url))
           sys (str "You are " voice " Rate the following motion-clip-generation prompt "
