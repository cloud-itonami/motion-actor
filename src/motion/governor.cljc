(ns motion.governor
  ...
  — not a semantic quality judge, motion.cosci already ran that; this re-checks
  hard invariants motion.judge/motion.cosci have no notion of: license
...
(defn violations
  ...
  motion.loop before this check) -> violation strings. Empty = passes."
  [asset]
  (cond-> []
