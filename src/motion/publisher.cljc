(ns motion.publisher
  ...
  createRecord via `motion.aozora`). Same shape as
  `cloud_itonami.media.publisher`."
  )

(def collection
  "A DISTINCT collection from every other actor's own stream (net.motion.*
...
(defrecord MockPublisher [a]
  ...
  (publish! [_ record]
    (swap! a conj record)
    {:uri (str "at://mock/gftd-motion-actor/" (hash record))
