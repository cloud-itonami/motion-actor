(ns motion.cosci
  "Google co-scientist / AlphaEvolve-shaped tournament over one round of motion
  clip candidates (ADR-2607122200 §3), ported-in-shape from
  ...
  slightly-worse asset, it's a disqualified one.
    ...
    Generation  -> motion.generate/round-candidates (closed gene pool)
    Reflection  -> hard gates: job actually :done, artifact fetched,
                   declared format matches, no upstream safety flag
    Ranking     -> Elo on motion.judge scores (pairwise, draw-tolerant)
...
(defn reflect
  ...
  :format \"edn\" :expected-format \"edn\" :safety-flag bool}."
...
(defn rank
  ...
  missing entries when motion.judge/score! failed) -> ranked vector,
...
(defn evolve-round
  "ranked -> {:next-round n+1 :elite-gene {...}} for motion.generate's next
