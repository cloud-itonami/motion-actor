(ns motion.datalad
  ...
  records them (motion.ledger) — so B2 isn't paid-for-and-polluted with
...
(defn ->isekai-manifest
  ...
  NOTE: `:asset/gen`'s `:stage` here is `:motion` — network-isekai's own
  `isekai.gen` maturity table (as of this writing) only wires up
  `:photo-to-3d`/`:diffusion`/`:tts`/`:music`, it has no `:motion` stage yet.
  `:motion` is a reasonable, forward-compatible stage key for this actor's
  output; it is NOT a confirmed existing stage name in network-isekai."
