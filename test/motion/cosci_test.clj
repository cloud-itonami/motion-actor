(ns motion.cosci-test
  (:require [clojure.test :refer [deftest testing is]]
            [motion.cosci :as cosci]))

(defn- result [id status & {:keys [format bytes safety] :or {format "edn" bytes (byte-array [1]) safety false}}]
  {:candidate {:candidate/id id :prompt (str "prompt-" id) :gene {:clip-type "c" :pacing "p"}}
   :status status :artifact-bytes (if (= status :done) bytes nil)
   :format (when (= status :done) format) :expected-format "edn" :safety-flag safety})
...
(deftest run-round-evolution-advances-round
  (let [{:keys [evolution]} (cosci/run-round [(result "a" :done)] {"a" {:score 80}} 5)]
    ...
    (is (= {:clip-type "c" :pacing "p"} (:elite-gene evolution)))))
